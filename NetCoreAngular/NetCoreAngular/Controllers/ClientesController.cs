...
namespace NetCoreAngular.Controllers
{
    ...
    public class ClientesController : ControllerBase
    {
        ...
        public IActionResult EditarCliente(ClienteViewmodel c)
        {
            ...
            try
            {
                ...
                using (CursoAngularNetCoreContext basedatos = new CursoAngularNetCoreContext())
                {
                    ...
                    cliente.Email = c.email;
                    cliente.Password = Encoding.ASCII.GetBytes(util.cifrar(c.pass, configuration["ClaveCifrado"]));
                    basedatos.Clientes.Add(cliente).State = Microsoft.EntityFrameworkCore.EntityState.Modified;
                    basedatos.SaveChanges();
                }
            ...
            {
                res.Error = "Se produjo un error al obtener al modificar un cliente" + ex.Message;
            }
            return Ok(res);
        }
        ...
        public IActionResult BorrarCliente(String Email)
        {
            ...
            {
                res.Error = "Se produjo un error al obtener al borrar un cliente" + ex.Message;
            }
            return Ok(res);
        }
